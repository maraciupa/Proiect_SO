...
//PHASE 2 EX3 FUNCTION //

void monitor_notification(char *district_id)
{
	//we need to have the pid:
	int fd=open(".monitor_pid", O_RDONLY);
	if(fd==-1)
	{
		//we need to write every error that occurs in the log file:
		//*role=>system, *user=>monitor, *action=>message about the monitor:
		logging_actions(district_id, "System", "Monitor", "MONITOR_NOTIFICATION: the hidden file couldn't be found!");
		return; //so that the report will be saved without stopping the program 
	}	
	char pids[50];
	int read_pid=read(fd, pids, sizeof(pids)-1);
	if(read_pid<=0)
	{
		logging_actions(district_id, "System", "Monitor", "MONITOR_NOTIFICATION: the PID couldn't be read!");
		return; //the same reason as above
	}
	//if the PID could be read:
	int pid=atoi(pids); //char=>int
	if(pid<=0) //meaning that the PID is not valid
	{
		logging_actions(district_id, "System", "Monitor", "MONITOR_NOTIFICATION: the PID is invalid!");
		return;
	}

	//trying to send the SIGUSR1 signal:
	if(kill(pid, SIGUSR1)==-1) //if kill did not work
	{
		logging_actions(district_id, "System", "Monitor", "MONITOR_NOTIFICATION: SIGUSR1 couldn't be send!");
		return;
	}
	
	//if there are not errors:
	logging_actions(district_id, "System", "Monitor", "MONITOR_NOTIFICATION: SUCCESS! SIGUSR1 was sent!");
	close(fd);
}
...
void add(char *district_id)
{
	...
	monitor_notification(district_id);
	close(fd);
}
